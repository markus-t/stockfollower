...
class port
{
  ...
  function getStock($dateLow, $dateHigh, $user = '1') {
    $output = array();
    $query="
  ...
  function getStockTransactions($stockID, $date) {
  ...
  function getQuantity($stockID, $date) {
    $output = array(
      "ID"         => $stockID,
      "aquantity"  => "0",
      "dquantity"  => "0",
    ...
    $listBoughtSold = port::getStockTransactions($stockID, $date);
  ...
  function getDividend($lowDate, $highDate, $dividend) {
    $output = array(
	         'sum' => "0");
    foreach($dividend as $row) {
	  ###fel funktion
	  $temp = port::getQuantity($row['stockID'], $row['date']);
	  $output['sum'] += $temp['aquantity'] * $row['price'];
    }
	return($output);
  }
	
  function getStockSpan($stockID, $user) {
  ...
  function getStockTransactionsOld($arr_stockID, $lowDate = '2000-01-01',$highDate = '2020-01-01'){
  $output = array();
    $fr = true;
    $qs = '';
  ...
  function simIndex($transactions, $indexID) {
    $output  = array();
    $ID      = '1';
    $a       = '0';
    $fT      = '2000-01-01';
    ...
    #Go trough every day
    while($fT <= $tT) {
      $indexPrice = index::getValue($fT, $indexID);
  ...
  function getStockSummary($lowDate, $highDate, $stockID) {
    $output = array(
	"id"          => "$stockID",
	"shortName"   => "",
	"name"        => "",
    ...
    $valueHigh = stock::getValue($stockID, $highDate);
    $valueLow  = stock::getValue($stockID, $lowDate);
    $output['date']   = $valueHigh['date'];
    $output['ltrade'] = $valueHigh['value'];
	
    $name = stock::resName($stockID);
    $output['shortName'] = $name['shortName'];
	$output['name']      = $name['name'];

	$QuantityHigh = port::getQuantity($stockID, $highDate);
	$output['q'] = $QuantityHigh['aquantity'] - $QuantityHigh['dquantity'];
	$output['aprice'] = $QuantityHigh['aprice'];
	
    $QuantityLow = port::getQuantity($stockID, $lowDate);
	$output['tq'] = $QuantityLow['aquantity'] - $QuantityLow['dquantity'];	

    $output['mvalue'] = $valueHigh['value'] * $output['q'];
    $output['diravkkr'] =  port::cacheGetDividendSum($lowDate, $highDate, $stockID);
    
    $sumLow  = port::cacheGetHoldingSum($lowDate, $stockID);
    $sumHigh = port::cacheGetHoldingSum($highDate, $stockID);
  ...
  function getStockActivityOld($lowDate, $highDate, $stockID) {
  ...
  function getStockAction($stockID, $date) {
  ...
  function cacheDividendSum() {
    global $daysInBankYear;
    $output = port::getStock('2011-05-01', '2013-01-01', "1");
    foreach($output as $stockID) {
      $dividend = stock::getDividendRange('2010-01-01', '2013-01-01', $stockID);
      if(!empty($dividend)) {
	    ##den det här ska bytas ut mot domnya type definitionera. Dividend type 2 calculations.
		if( $stockID <= 999) {
          foreach($dividend as $key){
            $tmValue = 0;
	        $qAtDividend = port::getQuantity($stockID, $key['date']);
          ...
          $dividendSum = 0;
		    while ($fromTime < $toTime) {
	          ...
	          $qAtDividend  = port::getQuantity($stockID, $fromTime);
  ...
  function cacheGetDividendSum($lowDate, $highDate, $stockID) {
  ...
  function cacheHoldingSum() {
    $span = port::getStock('2000-01-01', '2015-01-01', '1');
    $fT = '2011-05-01';
    $tT = '2013-01-01';
    $aAm = "0";
    $mAm = "0";
    ...
    foreach($span as $key) 
      while($fT <= $tT) {
        ...
        foreach($span as $key) {
          $rea = '0';
          $Quantity = port::getQuantity($key, $fT);
          $Value    = stock::getValue($key, $fT);
          ...
          $output['diravk']   =  port::cacheGetDividendSum('2010-01-01', $fT, $key);
  ...
  function cacheGetHoldingSum($date, $stockID) {
  ...
  function createTempTableHoldingSum() {
  ...
  function createTempTableDividendSum() {
