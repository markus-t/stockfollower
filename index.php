...
<th>Totalt</th>
 ...
 ?>
  ...
  foreach($stockList as $key) {
    $valueLow[$key] = port::cacheGetHoldingSum($FROM, $key);
...
<?php
          if($compareToIndex) {
            $arr = port::getStockTransactionsOld($stockList);
            $indexData = port::simIndex($arr, $indexISIN);
            $nameISIN =  index::resName($indexISIN);
...
<?php
foreach(port::getStock($FROM, $TO, "1") as $stockID) {
